...
- [x] **T0-11** Unblock CI/CD deploy — `iam.tf` ajoute `cicd_tfstate` binding sur le bucket (roles/storage.admin), `project.tf` ajoute `cloudresourcemanager.googleapis.com` aux APIs. Cascade de 3 root causes, 2 anticipés par Gemini review → **livré v1.0.7 (PR #38)**
