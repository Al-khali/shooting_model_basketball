...
- [x] **T0-15** Unblock bg pipeline live — `cpu_idle = false`, `YOLO_CONFIG_DIR`, `apt-get upgrade` (2 CVE HIGH fixed-upstream) (PR #40 / v1.0.9)
- [x] **T1-1 + T1-2** Track 1 complet : critique structurée 10 findings (3 HIGH étayés grep) + ADR-001 (5 décisions, 5 risques recensés) + révision doc qualif avec sections "Stack actuelle vs cible" et "Risques résiduels" (PRs #41 + T1-2 cette PR / v1.0.10). **Verdict** : ADK choice stands, AI Gateway à introduire (T3-1), Memory Bank en 2 paliers (T2-3 puis éventuel)
